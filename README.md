...
1. 从 Release 下载 `NoNameTag.dll` 和 `LiteDB.dll`。
2. 将 `NoNameTag.dll` 复制到服务器的 `Rocket/Plugins` 文件夹。
3. 将 `LiteDB.dll` 复制到服务器的 `Rocket/Libraries` 文件夹；如果该文件夹不存在请先创建。
4. 首次运行后编辑插件生成的配置，或参考 `NoNameTagConfiguration.example.xml`。
5. 重启服务器，或使用 Rocket 的重载命令。
